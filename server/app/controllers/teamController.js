...
class teamController {
  ...
  static async getUpdatedTeams(req, res) {
    const status=req.query.status
    try {
      const teams = await teamService.getUpdatedTeams(status);
      return res.json(Result.success(teams));
    } catch (error) {
      return res.json(Result.fail(error.message));
    }
  }
  static async getUpdatedTeamInfo(req, res) {
  ...
  static async getTeamActByCommu(req, res) {
    const commu_id = req.query.community_id;
    const status = req.query.status;
    try {
