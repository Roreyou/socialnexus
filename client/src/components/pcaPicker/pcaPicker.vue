  ...
  <view class="pcaPicker" @touchmove.stop.prevent="preventTouchMove">
    ...
    <view class="popBottom" :class="popFlag ? 'popRound' : ''">
      ...
      <picker-view :value="pcaIndex" @change="pcaChange">
        ...
        <picker-view-column>
          <block
            v-for="(item, index) in pcaArr[pcaIndex[0]].children"
            :key="index"
          >
            <view>{{ item.name }}</view>
          </block>
        ...
        <picker-view-column>
          <block
            v-for="(item, index) in pcaArr[pcaIndex[0]].children[pcaIndex[1]]
              .children"
            :key="index"
          >
            <view>{{ item.name }}</view>
...
export default {
  ...
  methods: {
    ...
    // 确认
    confirm(e) {
      let province = this.pcaArr[this.pcaIndex[0]].name;
      let city = this.pcaArr[this.pcaIndex[0]].children[this.pcaIndex[1]].name;
      let district =
        this.pcaArr[this.pcaIndex[0]].children[this.pcaIndex[1]].children[
          this.pcaIndex[2]
        ].name;
